...
2026-07-12 時点で、77項目中51項目（66%）が実装済み、26項目（34%）が計画中です。
...
| 実装済み | 51 | 章・コード・検証経路が存在する項目 |
| 計画中 | 26 | 説明または実装が未完成の項目 |
| production Scala | 約9,200行 | 標準ライブラリ中心の実装 |
| 宣言的テスト | 約5,100行、326 case | 正常・境界・失敗・数値propertyの証拠 |
| 英語正典 | 約70,000語 | 日本語版の翻訳元 |
...
| 領域 | 未実装項目 | 網羅に必要な成果物 |
| --- | --- | --- |
| 基礎 | 計算量、JVM system | allocation/locality lab、JIT/GC/concurrency/file/network lab |
| 数学 | 線形代数深掘り、統計的推測 | SVD/conditioning、推定・区間・calibration |
| tensor | execution model、精度engineering | broadcast/view/batch、float形式/loss scaling |
...
現在の51項目については、最小機構をコードから再構築し、failure pathをtestで観察する
学習経路を目標にしています。`sbt check` はformat、compile、326 case、英日対応、章構造、
...
したがって、正確な評価は「現在の✅範囲は証拠付きで広いが、カリキュラム全体は66%」
