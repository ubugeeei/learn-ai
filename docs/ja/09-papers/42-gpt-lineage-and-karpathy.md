...
`./learn-ai gpt-lineage`はinventoryと非互換理由を表示します。MiniGPTはRMSNorm、ReLU、
別initialization、course独自tokenizer/checkpointを使い、GPT-2 weight importerとdropoutも
ありません。shared mechanismを教えますが、GPT-2互換とは呼べません。
...
| [nanoGPT](https://github.com/karpathy/nanoGPT) | medium GPTのtrain/fine-tune/sample | 22a–d | tokenizer/checkpoint import |
| [build-nanogpt](https://github.com/karpathy/build-nanogpt) | GPT-2 124M再現とdistributed training | inventory、27/29章 | forward-logit parity |
...
現在は6を完成し、1–5をMiniGPTで代用せず明示的な未完了workにしました。
