...
loadしてlogit比較することで、tokenizer artifactもまだ未実装です。
...
現在は6と、2のdropout-off architecture部分を実装・test済みです。3はblock name/layoutまで
完了し、real whole-model containerは未完了です。1、4、5も未完了なので、真のGPT-2再現完了とは
まだ主張しません。
