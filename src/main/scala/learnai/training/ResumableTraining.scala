 ...
 *   - *scheduler*: `completedUpdates` — every schedule in this course is a pure function of the
 *     global update index, so the index is the state;
 *   - *data cursor*: `randomState` — batches are sampled with replacement from the RNG stream, so
 *     the stream position is the cursor;
 *   - *random state*: `randomState`, the full SplitMix64 counter.
 *
 * `initialValidationLoss` and the best-validation bookkeeping are carried so a resumed run reports
 * the same summary a straight run would. The state is an in-memory value; Chapter 25's versioned
 * checksummed format shows how to persist it, and doing so is this chapter's first exercise.
 */
...
object ResumableMiniGptTraining:
   ...
   *
   * The initial validation loss is computed here, exactly as the Chapter 22b trainer records it
  ...
  def freshState(
    ...
    val initialValidation = MiniGptTraining
   ...
   *
   * The state is restored into the supplied model before the first update, so resuming into a
   * freshly constructed (even differently initialized) model of the same architecture is exact. The
   * loop body is the Chapter 22b algorithm — replacement-sampled batches, weighted microbatch
   * accumulation, scheduled learning rates, interval validation — driven by a SplitMix64 stream
   * whose position travels inside the state.
   *
   * Exactness contract, verified bitwise by the test suite: for any split point `k`,
   * `train(_, s0, n)` produces the same metrics and final parameters as `train(_, s0, k)` followed
   * by `train(_, sk, n - k)`.
   *
   * @return
    ...
    while updateIndex < endIndex do
      ...
      val currentValidation =
        if shouldValidate then
          Some(MiniGptTraining.validationLoss(
            model,
            split.validation,
            config.batchSize,
   ...
   *
   * This is the resumable counterpart of `MiniGptTraining.train`. The two trainers intentionally
   * use different random generators, so their batch sequences — and therefore their metrics — are
   * not comparable run to run; each is deterministic under its own contract.
   */
  def trainFromStart(
      model: MiniGpt,
      split: CausalSplit,
  ...
  private def restoreParameters(parameters: Vector[Tensor], state: MiniGptTrainingState): Unit =
    require(
