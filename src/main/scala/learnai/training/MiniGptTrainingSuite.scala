  ...
  override val tests: Vector[TestCase] = specify(
    ...
    test("batch mean loss is the arithmetic mean of independently evaluated examples") {
      ...
      val expected = examples
        .map(example => model.loss(example.inputs, example.targets).valueAtFlat(0)).sum /
        examples.size.toDouble
      Assert
        .close(MiniGptTraining.meanLoss(model, batch).valueAtFlat(0), expected, tolerance = 1e-12)
    },
    ...
    test("training records scheduled validation tokens gradients and loss reduction") {
      ...
      Assert.isTrue(
        ...
        s"best validation ${run.bestValidationLoss} did not improve initial ${run
            .initialValidationLoss}"
      )
      Assert.isTrue(
        run.finalValidationLoss < run.initialValidationLoss * 0.5,
        s"final validation ${run.finalValidationLoss} was not below half of initial ${run
            .initialValidationLoss}"
      )
      Assert
        .isTrue(run.steps.forall(step => step.gradientNorm.isFinite && step.gradientNorm >= 0.0))
