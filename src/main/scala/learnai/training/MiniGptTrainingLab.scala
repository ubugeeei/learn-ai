...
def runMiniGptTrainingLab(): Unit =
  ...
  val config        = MiniGptTrainingConfig(
    ...
    learningRateSchedule =
      WarmupCosineLearningRate(peak = 0.02, minimum = 0.002, warmupUpdates = 5),
  ...
  println(
    f"best validation:     ${run.bestValidationLoss}%.6f at update ${run.bestValidationUpdate}%d"
  )
