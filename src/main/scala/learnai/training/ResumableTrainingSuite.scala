...
object ResumableTrainingSuite extends TestSuite:
  ...
  private def trainingConfig(totalUpdates: Int): MiniGptTrainingConfig = MiniGptTrainingConfig(
  ...
  override val tests: Vector[TestCase] = specify(
    test("every split point resumes bitwise-identically to the straight run") {
      ...
      val (straightRun, straightState) = ResumableMiniGptTraining
        .trainFromStart(straightModel, split, config)
      ...
      (1 until config.totalUpdates).foreach { splitPoint =>
        ...
        val (firstMetrics, middleState) = ResumableMiniGptTraining
          .train(firstModel, split, config, initial, splitPoint)
        ...
        val (secondMetrics, finalState) = ResumableMiniGptTraining
          .train(resumedModel, split, config, middleState, config.totalUpdates - splitPoint)
    ...
    test("the fresh state captures the untouched model and stream origin") {
      ...
      val expectedValidation = MiniGptTraining
    ...
    test("chunk boundaries do not change validation bookkeeping") {
      ...
      val (firstMetrics, middle) = ResumableMiniGptTraining.train(model, split, config, initial, 4)
      val (secondMetrics, _)     = ResumableMiniGptTraining.train(model, split, config, middle, 4)
      Assert
        .equal(firstMetrics.flatMap(step => step.validationLoss.map(_ => step.update)), Vector(3))
      Assert.equal(
    ...
    test("invalid chunk sizes states and architectures are rejected") {
      ...
      val widerModel        = MiniGpt
        .random(modelConfig.copy(channels = 8, hiddenChannels = 16), seed = 8L)
      ...
      val corruptBookkeeping = Assert
        .throws[IllegalArgumentException](state.copy(bestValidationUpdate = 1))
