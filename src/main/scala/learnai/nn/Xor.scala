...
final case class XorTrainingResult(
    model: MultiLayerPerceptron,
    lossHistory: Vector[Double]
)
...
object Xor:
  ...
  def predict(model: MultiLayerPerceptron): Vector[(LabeledPoint, Double)] =
    dataset.map(example => example -> model(example.inputs).head.data)

@main def trainXor(): Unit =
  ...
  Xor.predict(result.model).foreach { case (example, prediction) =>
    println(
      f"input=${example.inputs.mkString("[", ", ", "]")} " +
        f"target=${example.target}%4.1f prediction=$prediction% .6f"
    )
  }
