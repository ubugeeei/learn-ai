 ...
 *
 * `lossMask(t)` is true exactly when the prediction `inputs(t) -> targets(t)` should contribute
 * loss and gradient. Masked positions keep the window shape fixed — which batching requires — while
 * excluding padding targets and cross-document predictions from training.
 */
final case class PackedExample(
  ...
  /**
   * Number of positions that train the model. May be zero for degenerate windows (for example a
   * context of one whose only input is a separator); consumers should batch or filter with
 ...
 *
 * The counts exist so tests and experiment manifests can assert the packing arithmetic instead of
 * trusting it: `unmaskedTargetCount` must equal the total document token count, because every
 * document token is predicted exactly once (its predecessor predicts it, or, for a document's final
 * token, it predicts the separator).
 */
final case class PackingResult(
  ...
  def trainableExamples: Vector[PackedExample] = examples.filter(_.unmaskedTargetCount > 0)

object SequencePacking:
  /**
   * Packs variable-length documents into fixed windows with loss masks.
   *
   * The documents are concatenated in order, each terminated by `separator`, into one stream. The
   * stream is cut into consecutive windows of `contextLength + 1` tokens advancing by
   * `contextLength`, so every next-token prediction in the stream appears in exactly one window.
   * The tail is completed with `padding`.
   *
   * The mask at window position `t` is false when:
   *   - the target token is padding (the stream has ended), or
   *   - the input token is the separator — that prediction would teach the model to guess the *next
   *     document's* opening from the previous document's context.
   *
   * Predicting the separator itself stays unmasked: learning where documents end is real signal.
   * Note what packing does *not* do here: attention inside a window still crosses document
   * boundaries, because this course's attention has no block-diagonal document mask yet. That
   * contamination is bounded by the window length and is the standard trade-off of simple packing;
   * making it explicit is the point.
   *
   * Documents must not contain the separator or padding tokens, and the two must differ, so mask
  ...
  def pack(
    ...
    documents.zipWithIndex.foreach { case (document, index) =>
      ...
      require(!document.contains(separator), s"document $index contains the separator token")
      require(!document.contains(padding), s"document $index contains the padding token")
    }
