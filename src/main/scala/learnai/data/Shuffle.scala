...
/** A restart position inside an endless epoch-shuffled traversal.
  *
  * The cursor is the *entire* iteration state: the epoch selects a
  * permutation and the position indexes into it. Storing these two numbers
  * in a checkpoint is sufficient to continue a data sweep exactly, which is
  * the same resumability contract Chapter 22c establishes for training.
  */
...
object DeterministicShuffle:
  /** Fisher–Yates permutation of `[0, size)` driven by a SplitMix64 stream.
    *
    * The permutation is a pure function of `(size, seed, epoch)`. Each epoch
    * derives its own generator by offsetting the seed counter with the
    * golden gamma, the same stream-separation device SplitMix64 itself uses,
    * so consecutive epochs get well-mixed, mutually independent orders
    * without any O(epoch) skipping.
    */
...
/** An endless, deterministic, restartable shuffled sweep over example indices.
  *
  * Every epoch visits each of the `size` indices exactly once in an order
  * that changes per epoch. The sweep itself is immutable and stateless; all
  * iteration state lives in the caller-owned [[ShuffleCursor]], so two
  * consumers can traverse independently and a checkpointed cursor can be
  * resumed in another process.
  *
  * Permutations are recomputed per epoch rather than cached: the goal of
  * this reference implementation is an auditable contract, not throughput.
  */
final class ShuffledSweep(val size: Int, val seed: Long):
  ...
  /** Reads `count` consecutive indices and returns the cursor after them.
    *
    * The returned cursor makes reads composable:
    * `take(c, a + b)` equals `take(c, a)` followed by `take(after, b)`.
    */
  ...
  private def requireValid(cursor: ShuffleCursor): Unit =
    require(
      ...
      s"cursor position ${cursor.position} outside [0, $size)"
    )
