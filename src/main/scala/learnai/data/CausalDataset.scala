 ...
 *
 * `targets(t)` is the token immediately following `inputs(t)` in the source sequence. Inputs and
 * targets always have the same non-zero length.
 */
final case class CausalExample(inputs: Vector[TokenId], targets: Vector[TokenId]):
 ...
 *
 * The dataset is immutable. Batches may sample example indices with replacement using a
 * caller-owned random generator.
 */
final class CausalDataset private (val contextLength: Int, val examples: Vector[CausalExample]):
   ...
   */
  def sampleBatch(batchSize: Int, random: RandomGenerator): Either[String, CausalBatch] =
    ...
    else Right(CausalBatch(Vector.fill(batchSize)(examples(random.nextInt(examples.size)))))
  ...
  def sequentialBatches(batchSize: Int, dropLast: Boolean): Vector[CausalBatch] =
    require(batchSize > 0, s"batch size must be positive: $batchSize")
    examples.grouped(batchSize).filter(group => !dropLast || group.size == batchSize)
      .map(group => CausalBatch(group.toVector)).toVector
...
object CausalDataset:
  ...
  def fromTokens(tokens: Vector[TokenId], contextLength: Int): CausalDataset =
    ...
    val examples = tokens.sliding(contextLength + 1).filter(_.size == contextLength + 1)
      .map(window => CausalExample(window.take(contextLength), window.drop(1))).toVector
    new CausalDataset(contextLength, examples)

  /**
   * Splits raw tokens first, then creates windows independently.
   *
   * This ordering prevents any example from crossing the train/validation boundary. The exact
   * number of examples on each side depends on context length.
   */
  def contiguousSplit(
