...
object ShuffleSuite extends TestSuite:
  ...
  override val tests: Vector[TestCase] = specify(
    ...
    test("invalid sizes cursors and counts are rejected") {
      ...
      val outsideCursor = Assert
        .throws[IllegalArgumentException](sweep.indexAt(ShuffleCursor(epoch = 0L, position = 3)))
      Assert.isTrue(outsideCursor.getMessage.contains("outside"))
      val negativeCount = Assert
        .throws[IllegalArgumentException](sweep.take(ShuffleCursor(0L, 0), -1))
