...
/** The single public entrypoint for every executable lesson.
  *
  * Lesson functions remain close to the implementation they teach, while this
  * dispatcher makes discovery possible without knowing package names.
  */
object Main:
  private final case class Lesson(name: String, description: String, run: () => Unit)

  private val lessons = Vector(
    Lesson("foundations", "Scala values, collections, and explicit errors", () => runScalaTour()),
    Lesson("floating-point", "rounding error and stable numerical operations", () => runFloatingPointLab()),
    Lesson("calculus", "finite differences and analytical gradients", () => runGradientLab()),
    Lesson("gradient-descent", "fit a scalar parameter by optimization", () => runGradientDescentLab()),
    ...
    Lesson("training", "run batched MiniGPT training and validation", () => runMiniGptTrainingLab()),
    Lesson("diagnostics", "check gradients, parameter bytes, and timing", () => runMiniGptDiagnostics()),
  ...
  def main(arguments: Array[String]): Unit =
    arguments.toList match
      ...
      case command :: Nil =>
        lessons.find(_.name == command) match
  ...
  private def printHelp(): Unit =
    ...
    lessons.foreach { lesson =>
      println(f"  ${lesson.name}%-18s ${lesson.description}")
    }
    println("\nWorkflow:")
