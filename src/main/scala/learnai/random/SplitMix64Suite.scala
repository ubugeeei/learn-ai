...
object SplitMix64Suite extends TestSuite:
   ...
   *
   * Written directly from the algorithm description (advance by the golden gamma, then two
   * multiply-xorshift mixing rounds) rather than by calling the production class, so a
   * transcription error in either copy fails the comparison test.
   */
  ...
  override val tests: Vector[TestCase] = specify(
    ...
    test("bounded integers stay in range and reproduce across equal seeds") {
      ...
      Assert.equal(Vector.fill(100)(first.nextInt(13)), Vector.fill(100)(second.nextInt(13)))
    },
    ...
    test("doubles are uniform unit-interval values built from the top 53 bits") {
      ...
      values.foreach(value => Assert.isTrue(value >= 0.0 && value < 1.0, s"$value outside [0, 1)"))
