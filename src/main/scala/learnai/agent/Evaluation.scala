...
/** Requires byte-for-byte final-answer equality.
  *
  * Exact match is appropriate for deterministic protocol outputs. Semantic or
  * model-graded comparisons should be separate checks with explicit versions
...
final case class ExactAnswerCheck(expected: String) extends AgentRunCheck:
  ...
  override def evaluate(run: AgentRun): EvalCheckResult =
    EvalCheckResult(name, run.finalAnswer.contains(expected), s"expected='$expected', actual=${run.finalAnswer}")
...
final case class RequiredToolAttemptsCheck(required: Set[String]) extends AgentRunCheck:
  ...
  override def evaluate(run: AgentRun): EvalCheckResult =
    ...
    EvalCheckResult(name, missing.isEmpty, s"required=${required.toVector.sorted}, missing=${missing.toVector.sorted}")
...
final case class RequiredToolErrorCheck(code: String) extends AgentRunCheck:
  ...
  override def evaluate(run: AgentRun): EvalCheckResult =
    val observedCodes = run.history.collect {
      case ToolObservation(_, _, ToolFailed(error)) => error.code
    }.toSet
    EvalCheckResult(name, observedCodes.contains(code), s"required=$code, observed=${observedCodes.toVector.sorted}")
...
final case class MaximumTokensCheck(limit: Long) extends AgentRunCheck:
  ...
  override def evaluate(run: AgentRun): EvalCheckResult =
    EvalCheckResult(name, run.usage.totalTokens <= limit, s"limit=$limit, actual=${run.usage.totalTokens}")
...
final case class MaximumToolCallsCheck(limit: Int) extends AgentRunCheck:
  ...
  override def evaluate(run: AgentRun): EvalCheckResult =
    EvalCheckResult(name, run.executedToolCalls <= limit, s"limit=$limit, actual=${run.executedToolCalls}")
...
final case class AgentEvalCase(
  ...
  require(checks.map(_.name).distinct.size == checks.size, s"evaluation check names must be unique in '$name'")
...
final case class AgentEvalReport(results: Vector[AgentEvalResult]):
  ...
  val totalUsage: ModelUsage = results.foldLeft(ModelUsage.zero)((total, result) => total + result.run.usage)
  val totalLogicalToolCalls: Int = results.iterator.map(_.run.executedToolCalls).sum
  val totalModelSteps: Int = results.iterator.map(result => AgentEvaluation.modelSteps(result.run)).sum
...
object AgentEvaluation:
  def attemptedToolNames(run: AgentRun): Set[String] = run.events.collect {
    case ToolAttemptStarted(_, _, toolName, _) => toolName
  }.toSet

  def requestedToolNames(run: AgentRun): Set[String] = run.history.collect {
...
private final class EvalScriptedModel(decisions: Vector[Either[ModelError, ModelDecision]])
...
@main def runAgentEvaluationLab(): Unit =
  ...
  val cases = Vector(
    AgentEvalCase(
      ...
      Vector(ExpectedStatusCheck(AgentStatus.Completed), ExactAnswerCheck("42"), MaximumTokensCheck(30))
    ),
    AgentEvalCase(
      "required lookup",
      () => new EvalScriptedModel(
        Vector(
  ...
  report.results.foreach { result =>
    println(s"${result.name}: passed=${result.passed}, tokens=${result.run.usage.totalTokens}, " +
      f"latency=${result.durationNanos.toDouble / 1_000_000.0}%.3f ms")
