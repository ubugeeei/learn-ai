...
object AgentRuntimeSuite extends TestSuite:
  ...
  final private class ScriptedModel(decisions: Vector[Either[ModelError, ModelDecision]])
  ...
  final private class EchoTool(delayMillis: Long = 0L) extends Tool:
    var invocations                         = 0
    override val definition: ToolDefinition = ToolDefinition(
      "echo",
      "Returns the provided message.",
      ToolSchema(
        Vector(ToolField("message", JsonFieldType.StringValue, "Text to echo", required = true))
      )
    )
  ...
  final private class ControlledTool(
      effect: ToolEffect,
  ...
  override val tests: Vector[TestCase] = specify(
    ...
    test("a valid tool call becomes an observation for the next model step") {
      ...
      val model       = new ScriptedModel(Vector(
      ...
      val observation = model.requests(1).history.collectFirst { case value: ToolObservation =>
        value
      }
      Assert.equal(
        observation,
        Some(ToolObservation("call-1", "echo", ToolSucceeded(JsonString("hi"))))
      )
    },
    test("schema errors are returned to the model without invoking the tool") {
      ...
      val failure = run.history.collectFirst { case ToolObservation(_, _, ToolFailed(error)) =>
        error
      }
    ...
    test("unknown tools are capability failures visible to the model") {
      ...
      val error = run.history.collectFirst { case ToolObservation(_, _, ToolFailed(problem)) =>
        problem
      }
    ...
    test("tool timeout is a retryable observation and interrupts the task") {
      ...
      val model   = new ScriptedModel(Vector(
        Right(RequestTools(Vector(call), smallUsage)),
        Right(FinalAnswer("fallback", smallUsage))
      ))
      val run     = new AgentRuntime(Vector(tool), AgentConfig(toolTimeoutMillis = 10L))
    ...
    test("effectful tools are denied unless the host grants approval") {
      val tool          =
        new ControlledTool(ToolEffect.IdempotentWrite, Vector(Right(JsonString("written"))))
      val call          = ToolCall("write-1", "controlled", JsonObject.empty)
      val model         = new ScriptedModel(Vector(
        Right(RequestTools(Vector(call), smallUsage)),
        Right(FinalAnswer("not written", smallUsage))
      ))
      val run           = new AgentRuntime(Vector(tool), AgentConfig()).run(model, userHistory)
      val error         = run.history.collectFirst { case ToolObservation(_, _, ToolFailed(problem)) =>
        problem
    ...
    test("an approved idempotent write executes with an auditable decision") {
      ...
      val run   = new AgentRuntime(Vector(tool), AgentConfig(), ToolApprover.allowAll)
        .run(model, userHistory)
      Assert.equal(tool.invocations, 1)
    ...
    test("retryable read-only failures use distinct bounded attempts") {
      ...
      val run   = new AgentRuntime(Vector(tool), AgentConfig(maximumToolAttempts = 2))
        .run(model, userHistory)
    ...
    test("non-idempotent writes are never retried automatically") {
      ...
      val model = new ScriptedModel(Vector(
        Right(RequestTools(Vector(call), smallUsage)),
        Right(FinalAnswer("stopped", smallUsage))
      ))
      val run   =
        new AgentRuntime(Vector(tool), AgentConfig(maximumToolAttempts = 3), ToolApprover.allowAll)
          .run(model, userHistory)
      Assert.equal(tool.invocations, 1)
    ...
    test("an approver exception fails closed without invoking the tool") {
      ...
      val model           = new ScriptedModel(Vector(
        Right(RequestTools(Vector(call), smallUsage)),
        Right(FinalAnswer("safe", smallUsage))
      ))
      val run             = new AgentRuntime(Vector(tool), AgentConfig(), failingApprover)
        .run(model, userHistory)
      Assert.equal(tool.invocations, 0)
      Assert.isTrue(run.history.exists {
        case ToolObservation(_, _, ToolFailed(error)) => error.code == "approval_denied" &&
          error.message.contains("approver failed")
        case _                                        => false
      })
    },
    test("tool-call budget stops before an unapproved extra execution") {
      ...
      val run   = new AgentRuntime(Vector(tool), AgentConfig(maximumToolCalls = 1))
        .run(model, userHistory)
    ...
    test("model-step limit terminates a loop with a complete trace") {
      ...
      val run   = new AgentRuntime(Vector.empty, AgentConfig(maximumModelSteps = 1))
        .run(model, userHistory)
      Assert.equal(run.status, AgentStatus.ModelStepLimitExceeded)
      Assert.equal(
        run.events.last.asInstanceOf[AgentStopped].status,
    ...
    test("model errors terminate without being converted into tool observations") {
      ...
      Assert
        .isTrue(run.events.last.asInstanceOf[AgentStopped].reason.contains("provider_unavailable"))
    ...
    test("schema validation reports required unknown and integer type failures together") {
      ...
      val problems = schema
        .validate(JsonObject("count" -> JsonNumber(BigDecimal("1.5")), "extra" -> JsonString("no")))
