...
object PlanningSuite extends TestSuite:
  ...
  final private class OneDecisionModel(decision: Either[ModelError, ModelDecision])
  ...
  final private class RecordingFactory(
  ...
  private def plan(goal: String, tasks: Vector[PlanTask]): TaskPlan = Assert
    .right(TaskPlan.create(goal, tasks))
  ...
  override val tests: Vector[TestCase] = specify(
    test("plan validation rejects duplicate missing and cyclic dependencies") {
      val duplicate = TaskPlan
        .create("goal", Vector(PlanTask("same", "first"), PlanTask("same", "second")))
      val missing   = TaskPlan.create("goal", Vector(PlanTask("task", "work", Vector("unknown"))))
      val cyclic    = TaskPlan.create(
        "goal",
        Vector(PlanTask("a", "first", Vector("b")), PlanTask("b", "second", Vector("a")))
      )
    ...
    test("ready tasks execute in declared dependency order") {
      ...
      val result  = new PlanningAgent(runtime, PlanningConfig())
        .run(graph, Vector(TextMessage(MessageRole.User, "start")), factory)
      ...
      Assert.equal(factory.contexts.last.dependencyAnswers, Map("collect" -> "answer-collect"))
    ...
    test("failed branches block descendants but not independent work") {
      ...
      val factory  = new RecordingFactory(context =>
        if context.task.id == "fail" then Left(ModelError("permanent", "stop", retryable = false))
        else Right(FinalAnswer("independent answer", ModelUsage(1, 1)))
      )
      val result   = new PlanningAgent(runtime, PlanningConfig(maximumTaskAttempts = 1))
        .run(graph, Vector.empty, factory)
    ...
    test("a dependency-closed checkpoint skips completed task effects") {
      ...
      val checkpoint = Assert.right(PlanCheckpoint.create(graph, Map("first" -> "durable answer")))
      val factory    =
        new RecordingFactory(context => Right(FinalAnswer("new answer", ModelUsage(1, 1))))
      val result     = new PlanningAgent(runtime, PlanningConfig())
        .run(graph, Vector.empty, factory, checkpoint)
    ...
    test("checkpoint validation rejects unknown and dependency-incomplete state") {
      val graph      = plan(
        "validate resume",
        Vector(PlanTask("first", "first"), PlanTask("second", "second", Vector("first")))
      )
    ...
    test("dependency outputs are labeled as untrusted conversation data") {
      ...
      val downstream = histories.last.collect { case TextMessage(_, content) => content }
        .mkString("\n")
