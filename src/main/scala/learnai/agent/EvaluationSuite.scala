...
object EvaluationSuite extends TestSuite:
  ...
  final private class RecordingTool(effect: ToolEffect = ToolEffect.ReadOnly) extends Tool:
    var invocations                         = 0
    override val definition: ToolDefinition =
      ToolDefinition("record", "Records one invocation.", ToolSchema(Vector.empty), effect)
  ...
  override val tests: Vector[TestCase] = specify(
    test("a report aggregates deterministic outcome trajectory and cost checks") {
      ...
      val evalCase = AgentEvalCase(
        ...
        () =>
          new ScriptedModel(
            Vector(Right(RequestTools(Vector(call), usage)), Right(FinalAnswer("done", usage)))
          ),
        initialHistory,
        Vector(
      ...
      val report   = new AgentEvaluator(new AgentRuntime(Vector(tool), AgentConfig()), pricing)
    ...
    test("failed checks remain visible by case and category") {
      val evalCase = AgentEvalCase(
        ...
        initialHistory,
        Vector(ExactAnswerCheck("expected"), MaximumTokensCheck(1))
      )
      val report   = new AgentEvaluator(new AgentRuntime(Vector.empty, AgentConfig()))
        .evaluate(Vector(evalCase))
    ...
    test("requested and physically attempted forbidden tools are distinct checks") {
      ...
      val result   = new AgentEvaluator(new AgentRuntime(Vector(tool), AgentConfig()))
    ...
    test("duplicate case and check names fail before model execution") {
      ...
      val valid           = AgentEvalCase("same", factory, Vector.empty, Vector(ExactAnswerCheck("ok")))
      val duplicateCases  = Assert.throws[IllegalArgumentException] {
        new AgentEvaluator(new AgentRuntime(Vector.empty, AgentConfig()))
          .evaluate(Vector(valid, valid))
      }
