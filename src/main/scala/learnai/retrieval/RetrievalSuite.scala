...
object RetrievalSuite extends TestSuite:
  ...
  private val documents = Vector(
    ...
    SourceDocument(
      "garden",
      "Garden",
      "Tomatoes need sunlight and regular watering in the garden."
    ),
  ...
  override val tests: Vector[TestCase] = specify(
    test("chunker covers the source with the configured overlap") {
      ...
      Assert.equal(
        chunks.map(chunk => chunk.startOffset -> chunk.endOffset),
        Vector(0 -> 4, 3 -> 7, 6 -> 10, 9 -> 10)
      )
    ...
    test("search order is deterministic when scores tie") {
      val tied    =
        Vector(SourceDocument("b", "B", "same words"), SourceDocument("a", "A", "same words"))
    ...
    test("search tool exposes citation metadata in structured JSON") {
      ...
      val output = Assert.right(tool.execute(
