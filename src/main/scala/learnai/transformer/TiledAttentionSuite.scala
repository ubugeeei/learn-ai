...
object TiledAttentionSuite extends TestSuite:
  ...
  private def referenceAttendRow(
    ...
    val weights = referenceSoftmax(keys.map(key => key.zip(query).map(_ * _).sum * scale))
  ...
  private def randomRows(
      ...
      rows: Int,
      channels: Int
  ): Vector[Vector[Double]] = Vector.fill(rows)(Vector.fill(channels)(random.nextDouble(-1.0, 1.0)))
  ...
  override val tests: Vector[TestCase] = specify(
    ...
    test("folding tiles in any split yields one normalizer state") {
      ...
      val split   = scores.grouped(2)
        .foldLeft(TiledAttention.OnlineSoftmaxState.empty) { (state, tile) =>
          TiledAttention.foldTile(state, tile.toVector)
        }
      val singles = scores.foldLeft(TiledAttention.OnlineSoftmaxState.empty) { (state, score) =>
        TiledAttention.foldTile(state, Vector(score))
      }
    ...
    test("tiled causal attention matches the materializing Tensor path") {
      ...
      val reference   = queryTensor.matmul(keyTensor.transpose2D).scale(scale).causalMask()
        .softmaxRows.matmul(valueTensor)
    ...
    test("causality holds by construction: future rows cannot leak backwards") {
      ...
      val perturbed     = TiledAttention
        .causalAttentionTiled(queries, changedKeys, changedValues, scale, 2)
      (0 until 3).foreach(row => Assert.equal(perturbed(row), original(row)))
    },
    ...
    test("invalid tiles scores and shapes are rejected") {
      ...
      val zeroTile       = Assert
        .throws[IllegalArgumentException](TiledAttention.softmaxStreamed(Vector(1.0), tileSize = 0))
