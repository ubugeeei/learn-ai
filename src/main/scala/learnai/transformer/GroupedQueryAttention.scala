 ...
 *
 * Standard multi-head attention (Chapter 19) gives every query head its own key and value
 * projections. During cached inference (Chapter 24) the KV cache — not the weights — dominates
 * memory, and its size is proportional to the number of key/value heads. Grouped-query attention
 * (GQA) keeps all query heads but shares each key/value head across a *group* of consecutive query
 * heads:
 *
 * ```text
 ...
 *   - `keyValueHeadCount == queryHeadCount` is exactly multi-head attention (the test suite proves
 *     output equality against Chapter 19);
 *   - `keyValueHeadCount == 1` is multi-query attention (MQA).
 *
 * Cached KV payload per token shrinks from `2 * channels` values to
 * `2 * keyValueHeadCount * headChannels`, a factor of `queryHeadCount / keyValueHeadCount`;
 * [[keyValuePayloadBytesPerToken]] exposes the exact accounting so serving-memory claims can be
...
final class GroupedQueryAttention private (
   ...
   *
   * Every query head computes its own scores and weighted values, but reads keys and values from
   * the channel slice of its owning key/value head.
   */
   ...
   *
   * A Chapter 24 style cache stores one key row and one value row per layer and token. Under GQA
   * those rows are `keyValueChannels` wide, so the per-token payload is `2 * keyValueChannels * 8`
   * bytes instead of multi-head attention's `2 * channels * 8`.
   */
  def keyValuePayloadBytesPerToken: Long = 2L * keyValueChannels.toLong *
...
object GroupedQueryAttention:
   ...
   *
   * This factory exists for equivalence oracles: a grouped layer built from a Chapter 19 layer's
   * projections with `keyValueHeadCount == queryHeadCount` must reproduce its output exactly. All
   * shape invariants are re-validated by the constructor.
   */
  def fromProjections(
      channels: Int,
      queryHeadCount: Int,
  ...
  ): GroupedQueryAttention = new GroupedQueryAttention(
    channels,
    queryHeadCount,
    keyValueHeadCount,
    queryProjection,
