...
/** Decoder-only GPT architecture values needed to reproduce parameter inventory.
  *
  * This contract follows GPT-2/nanoGPT naming: learned token and position
  * embeddings, pre-LayerNorm blocks, biased QKV/output projections, a 4x GELU
  * MLP, final LayerNorm, and an output head tied to token embeddings.
  */
final case class Gpt2Config(
  ...
  require(headCount > 0 && channels % headCount == 0, s"$headCount heads must divide $channels channels")
...
final case class Gpt2ParameterInventory(
  ...
  val perLayer: Long = Math.addExact(Math.addExact(attentionPerLayer, feedForwardPerLayer), normsPerLayer)
