  ...
  override val tests: Vector[TestCase] = specify(
    test("equal query and key/value head counts reproduce multi-head attention") {
      ...
      standardResult.weightsByHead.zip(groupedResult.weightsByHead).foreach {
        case (expected, actual) => expected.values.zip(actual.values)
            .foreach { case (left, right) => Assert.close(right, left, tolerance = 1e-12) }
      }
    },
    test("grouped sharing equals full-width attention with duplicated key/value columns") {
      ...
      def randomValues(count: Int): Vector[Double] = Vector
        .fill(count)(random.nextDouble(-0.5, 0.5))

      val queryProjection  = Linear
        .fromValues(channels, channels, randomValues(64), randomValues(8), "query")
      val outputProjection = Linear
        .fromValues(channels, channels, randomValues(64), randomValues(8), "output")
      ...
      def expandRow(row: Vector[Double]): Vector[Double]         = row.grouped(headChannels).toVector
        .flatMap(block => Vector.fill(groupSize)(block).flatten)
      def expandWeights(weights: Vector[Double]): Vector[Double] = weights.grouped(keyValueChannels)
        .toVector.flatMap(expandRow)
      ...
      val expanded = GroupedQueryAttention.fromProjections(
        ...
        queryProjection,
        Linear
          .fromValues(channels, channels, expandWeights(keyWeights), expandRow(keyBiases), "key"),
        Linear.fromValues(
          channels,
          channels,
          expandWeights(valueWeights),
          expandRow(valueBiases),
          "value"
        ),
        outputProjection
      )
    ...
    test("multi-query attention shares a single key/value head across all queries") {
      val attention = GroupedQueryAttention.random(8, 4, 1, new SplittableRandom(4L), "mqa")
      ...
      val fullWidth = GroupedQueryAttention.random(8, 4, 4, new SplittableRandom(4L), "mha")
      Assert
        .equal(fullWidth.keyValuePayloadBytesPerToken, attention.keyValuePayloadBytesPerToken * 4L)
    },
    test("causal masking holds for every query head") {
      val attention = GroupedQueryAttention.random(8, 4, 2, new SplittableRandom(5L), "gqa")
    ...
    test("future inputs receive no gradient from a prefix-only loss") {
      val attention         = GroupedQueryAttention.random(4, 2, 1, new SplittableRandom(7L), "gqa")
    ...
    test("parameter and cache-byte accounting follow the closed formulas") {
      val channels         = 12
      val attention        = GroupedQueryAttention.random(channels, 6, 2, new SplittableRandom(8L), "gqa")
      val keyValueChannels = 2 * (channels / 6)
      Assert.equal(attention.keyValueChannels, keyValueChannels)
      Assert.equal(
        attention.parameterCount,
        2 * (channels * channels + channels) + 2 * (channels * keyValueChannels + keyValueChannels)
      )
      Assert.equal(attention.keyValuePayloadBytesPerToken, 2L * keyValueChannels.toLong * 8L)
      Assert.equal(
        attention.cachePayloadBytes(contextLength = 128, layerCount = 4),
        attention.keyValuePayloadBytesPerToken * 128L * 4L
      )
      val fullWidth        = GroupedQueryAttention.random(channels, 6, 6, new SplittableRandom(8L), "mha")
      Assert.isTrue(
