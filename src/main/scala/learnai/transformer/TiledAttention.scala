 ...
 *
 * Standard attention materializes a full `[time, time]` score matrix, and Chapter 29a showed that
 * this quadratic term dominates memory at long contexts. FlashAttention's observation is that
 * softmax can be computed *incrementally*: scores can arrive in tiles, and a small running state —
 * maximum, denominator, and an unnormalized value accumulator — is enough to produce the exact same
 * output without ever holding a full score row.
 *
 * This module is a faithful CPU simulation of that algorithm, not a fused kernel: it demonstrates
 * and tests the mathematics (the rescaling recurrence and its exactness) while the memory win on
 * real hardware comes from executing the same recurrence inside on-chip SRAM. It is forward-only by
 * design; the trainable path remains Chapter 19's, and equality with it is this module's
 * correctness oracle.
 *
 ...
 * ```
 *
 * When a tile with local maximum m' arrives, previous contributions are rescaled by
 * `exp(m - max(m, m'))`, which is always in (0, 1]; nothing is ever exponentiated above zero, so
 * extreme scores cannot overflow.
 */
object TiledAttention:
  ...
  object OnlineSoftmaxState:
    ...
    val empty: OnlineSoftmaxState = OnlineSoftmaxState(Double.NegativeInfinity, 0.0)

  /**
   * Folds one tile of scores into the running softmax state.
   *
   * This is the exact FlashAttention recurrence for the normalizer: `m'' = max(m, tileMax)` and
  ...
  def foldTile(state: OnlineSoftmaxState, tile: Vector[Double]): OnlineSoftmaxState =
    ...
    val rescaledPrevious =
      if state.denominator == 0.0 then 0.0 else state.denominator * math.exp(state.maximum - merged)
   ...
   *
   * One pass builds the normalizer state; a second pass normalizes. A fused kernel avoids the
   * second pass over memory by rescaling its output accumulator instead — that is
   * [[attendRowTiled]].
   */
  def softmaxStreamed(scores: Vector[Double], tileSize: Int): Vector[Double] =
    ...
    val state = scores.grouped(tileSize)
   ...
   *
   * `keys` and `values` hold one row per attendable position (the causal prefix including the
   * query's own position). The full score row is never materialized: at most `tileSize` scores plus
   * one channel-width accumulator exist at any moment, which is the memory argument
   * [[materializedFloatsPerRow]] quantifies.
   */
  def attendRowTiled(
      query: Vector[Double],
  ...
  /**
   * Full causal attention over `[time, channels]` rows, one tiled pass per query row. Row `t`
   * attends to positions `0..t`, so causality holds by construction rather than by masking a
   * materialized square matrix.
   */
  def causalAttentionTiled(
    ...
    Vector.tabulate(queries.size) { time =>
      attendRowTiled(queries(time), keys.take(time + 1), values.take(time + 1), scale, tileSize)
    }

  /**
   * Peak per-position floats one tiled query row materializes: `tileSize` scores plus the
   * channel-wide accumulator and two scalars. The untiled row holds all `contextLength` scores
   * instead — the quadratic term of Chapter 29a once multiplied by rows, heads, and layers.
   */
