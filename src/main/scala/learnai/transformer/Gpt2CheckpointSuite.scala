...
object Gpt2CheckpointSuite extends TestSuite:
  ...
  private def fixture(channels: Int): Map[String, Gpt2CheckpointTensor] =
    ...
    def values(size: Int, start: Int): Vector[Double]        = Vector
      .tabulate(size)(index => (start + index).toDouble / 100.0)
    Map(
      ...
      s"$prefix.attn.c_attn.weight" ->
        Gpt2CheckpointTensor(Shape(channels, 3 * channels), values(3 * channels * channels, 1)),
      s"$prefix.attn.c_attn.bias"   ->
        Gpt2CheckpointTensor(Shape(3 * channels), values(3 * channels, 30)),
      s"$prefix.attn.c_proj.weight" ->
        Gpt2CheckpointTensor(Shape(channels, channels), values(channels * channels, 40)),
      ...
      s"$prefix.mlp.c_fc.weight"    ->
        Gpt2CheckpointTensor(Shape(channels, 4 * channels), values(4 * channels * channels, 60)),
      s"$prefix.mlp.c_fc.bias"      ->
        Gpt2CheckpointTensor(Shape(4 * channels), values(4 * channels, 90)),
      s"$prefix.mlp.c_proj.weight"  ->
        Gpt2CheckpointTensor(Shape(4 * channels, channels), values(4 * channels * channels, 110)),
