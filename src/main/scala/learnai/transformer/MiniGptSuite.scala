  ...
  override val tests: Vector[TestCase] = specify(
    ...
    test("training backpropagates through the full model and lowers loss") {
      ...
      val history = MiniGptTrainer
        .trainSequence(model, inputs, targets, steps = 120, learningRate = 0.03)
    ...
    test("cached generation matches the reference across context rebuilds") {
      ...
      val reference = Assert
        .right(model.generate(prompt, 3, temperature = 0.9, new SplittableRandom(11L)))
      val cached    = Assert.right(model.generateCached(
        prompt,
        newTokenCount = 3,
    ...
    test("KV caching reduces deterministic token work before capacity") {
      ...
      val result = Assert.right(model.generateCached(
    ...
    test("inference sessions make capacity and reset ownership explicit") {
      ...
      val overflow = Assert.throws[IllegalArgumentException](session.append(TokenId(0)))
    ...
    test("invalid configuration sequence and target values fail near the boundary") {
      val configError = Assert.throws[IllegalArgumentException](config.copy(channels = 5))
      val model       = MiniGpt.random(config, seed = 10L)
      val lengthError = Assert
        .throws[IllegalArgumentException](model.logits(Vector.fill(5)(TokenId(0))))
      val targetError = Assert
        .throws[IllegalArgumentException](model.loss(Vector(TokenId(0)), Vector(TokenId(5))))
