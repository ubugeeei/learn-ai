...
object TransformerBlockSuite extends TestSuite:
  ...
  private def create(seed: Long): TransformerBlock = TransformerBlock.random(
    channels = 4,
    headCount = 2,
    hiddenChannels = 8,
    epsilon = 1e-5,
  ...
  override val tests: Vector[TestCase] = specify(
    ...
    test("a scalar loss sends finite gradients to input and every parameter") {
      ...
      val input = Tensor
        .parameter(Shape(3, 4), Vector.tabulate(12)(index => index * 0.03 + 0.1), "input")
      ...
      block.parameters.foreach { parameter =>
        Assert.isTrue(
          parameter.gradients.forall(_.isFinite),
          s"non-finite gradient in ${parameter.label}"
        )
      }
    ...
    test("the complete block preserves prefix causality") {
      ...
      val first   = Tensor
        .constant(Shape(3, 4), prefix ++ Vector(5.0, 6.0, 7.0, 8.0, 9.0, 10.0, 11.0, 12.0))
      val changed = Tensor.constant(
        Shape(3, 4),
        prefix ++ Vector(-50.0, 60.0, -70.0, 80.0, 90.0, -100.0, 110.0, -120.0)
      )
      block(first).rowValues(0).zip(block(changed).rowValues(0)).foreach { case (left, right) =>
    ...
    test("invalid feed-forward width is rejected") {
      val error = Assert.throws[IllegalArgumentException] {
        TransformerBlock
          .random(4, 2, hiddenChannels = 0, epsilon = 1e-5, new SplittableRandom(1L), "block")
      }
