...
object LayersSuite extends TestSuite:
  ...
  override val tests: Vector[TestCase] = specify(
    ...
    test("token and position embeddings make repeated tokens position-dependent") {
      val tokenEmbedding = Embedding.fromValues(
        2,
        2,
        Vector(1.0, 2.0, 3.0, 4.0),
        "token"
      )
      val positionEmbedding = Embedding.fromValues(
        3,
        2,
        Vector(0.0, 0.0, 10.0, 20.0, 100.0, 200.0),
        "position"
      )
    ...
    test("embedding rejects out-of-range IDs and excessive context") {
      ...
      val lengthError = Assert.throws[IllegalArgumentException] {
        combined(Vector(TokenId(0), TokenId(1), TokenId(2)))
      }
