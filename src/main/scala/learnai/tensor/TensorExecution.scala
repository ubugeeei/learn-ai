...
final case class BroadcastPlan private (
  ...
  def map(leftValues: IndexedSeq[Double], rightValues: IndexedSeq[Double])(
    ...
    Vector.tabulate(output.size)(index =>
      operation(leftValues(leftOffsets(index)), rightValues(rightOffsets(index)))
    )
...
object BroadcastPlan:
  ...
  def between(left: Shape, right: Shape): Either[String, BroadcastPlan] =
    ...
    val dimensions      = leftDimensions.zip(rightDimensions).zipWithIndex
      .foldLeft[Either[String, Vector[Int]]](Right(Vector.empty)) {
    ...
    dimensions.map { outputDimensions =>
      ...
      val offsets       = Vector.tabulate(output.size) { flat =>
        ...
        outputDimensions.indices.foreach { axis =>
          val coordinate =
            if outputDimensions(axis) == 0 then 0 else remainder / outputStrides(axis)
  ...
  private def rowMajorStrides(dimensions: Vector[Int]): Vector[Int] = dimensions.indices
    .map(axis => dimensions.drop(axis + 1).product).toVector
...
final case class StridedView private (
  ...
  def transpose(first: Int, second: Int): StridedView =
    ...
    StridedView(
      backing,
      Shape(shape.dimensions.updated(first, shape(second)).updated(second, shape(first))*),
      offset,
      strides.updated(first, strides(second)).updated(second, strides(first))
    )

  /** Materializes logical row-major order, making the copy boundary observable. */
  def materialize: Vector[Double] = Vector.tabulate(shape.size) { flat =>
    var remainder = flat
...
object StridedView:
  ...
  def contiguous(backing: Vector[Double], shape: Shape): StridedView =
    ...
    val strides = shape.dimensions.indices.map(axis => shape.dimensions.drop(axis + 1).product)
      .toVector
...
object BatchedMatmul:
  def apply(left: Tensor, right: Tensor): Tensor =
    ...
    val values  = Vector.tabulate(batch * rows * columns) { flat =>
      ...
      (0 until inner).map(index => left(batchIndex, row, index) * right(batchIndex, index, column))
        .sum
    }
...
object GraphLifetime:
  def retain(shapes: Vector[Shape], bytesPerElement: Int): GraphLifetime =
    ...
    val bytes = shapes.foldLeft(0L)((total, shape) =>
      Math.addExact(total, Math.multiplyExact(shape.size.toLong, bytesPerElement.toLong))
    )
