...
final case class EigenEstimate(value: Double, vector: VectorD, iterations: Int, residualNorm: Double)
...
object LinearAlgebra:
  ...
  def dominantEigenpair(
    ...
    require(matrix.rows == matrix.columns && matrix.rows > 0, "power iteration requires a non-empty square matrix")
  ...
  def outer(left: VectorD, right: VectorD): MatrixD =
    MatrixD.tabulate(left.size, right.size)((row, column) => left(row) * right(column))
