...
object Numerics:
  ...
  def naiveSum(values: IterableOnce[Double]): Double =
    values.iterator.foldLeft(0.0)(_ + _)

  /** Kahan compensated summation keeps a running estimate of bits lost to
    * rounding. It does not make floating-point arithmetic exact, but it often
    * reduces error when magnitudes differ.
    */
