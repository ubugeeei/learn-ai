...
final class MatrixD private (val rows: Int, val columns: Int, private val values: Array[Double]):
  val size: Int = values.length

  def apply(row: Int, column: Int): Double = values(offset(row, column))
  ...
  def transpose: MatrixD = MatrixD.tabulate(columns, rows)((row, column) => apply(column, row))
  ...
  def matvec(vector: VectorD): VectorD =
    ...
    VectorD.tabulate(rows)(rowIndex => dotCell(rowIndex, vector))

  /** Matrix multiplication: [m, k] x [k, n] -> [m, n]. */
  def matmul(other: MatrixD): MatrixD =
    require(
      columns == other.rows,
      s"matmul shape mismatch: left=[$rows,$columns], " + s"right=[${other.rows},${other.columns}]"
    )
  ...
  def toRows: Vector[Vector[Double]] = Vector.tabulate(rows)(row(_).toVector)

  override def equals(other: Any): Boolean = other match
    case that: MatrixD => rows == that.rows && columns == that.columns &&
      Arrays.equals(values, that.values)
    case _             => false

  override def hashCode(): Int = 31 * (31 * rows + columns) + Arrays.hashCode(values)

  override def toString: String = toRows.map(_.mkString("[", ", ", "]"))
    .mkString("MatrixD(", ", ", ")")
  ...
  private def requireSameShape(other: MatrixD): Unit = require(
    rows == other.rows && columns == other.columns,
    s"matrix shape mismatch: left=[$rows,$columns], " + s"right=[${other.rows},${other.columns}]"
  )
...
object MatrixD:
  ...
  private def fromOwnedArray(rows: Int, columns: Int, values: Array[Double]): MatrixD =
    var index = 0
  ...
  private[math] def unsafeFromOwnedArray(rows: Int, columns: Int, values: Array[Double]): MatrixD =
