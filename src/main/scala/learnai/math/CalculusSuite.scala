...
object CalculusSuite extends TestSuite:
  ...
  override val tests: Vector[TestCase] = specify(
    ...
    test("directional derivative agrees with gradient dot unit direction") {
      ...
      val directional = Assert.right(Calculus.directionalDerivative(function, point, direction))
      Assert.close(directional, 6.0, tolerance = 1e-8)
    },
    test("finite differences reject invalid steps") {
      val zero      = Assert
        .throws[IllegalArgumentException](Calculus.derivative(identity, 1.0, step = 0.0))
      val nonFinite = Assert
        .throws[IllegalArgumentException](Calculus.derivative(identity, 1.0, step = Double.NaN))
