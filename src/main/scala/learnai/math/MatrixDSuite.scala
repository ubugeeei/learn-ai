...
object MatrixDSuite extends TestSuite:
  private val matrix = MatrixD.fromRows(Vector(VectorD(1.0, 2.0, 3.0), VectorD(4.0, 5.0, 6.0)))
  ...
  override val tests: Vector[TestCase] = specify(
    ...
    test("transpose swaps rows and columns") {
      val expected = MatrixD
        .fromRows(Vector(VectorD(1.0, 4.0), VectorD(2.0, 5.0), VectorD(3.0, 6.0)))
    ...
    test("matrix multiplication produces the expected shape and values") {
      val right    = MatrixD
        .fromRows(Vector(VectorD(7.0, 8.0), VectorD(9.0, 10.0), VectorD(11.0, 12.0)))
      val expected = MatrixD.fromRows(Vector(VectorD(58.0, 64.0), VectorD(139.0, 154.0)))
      Assert.equal(matrix.matmul(right), expected)
    },
    test("matrix multiplication rejects incompatible inner dimensions") {
      val error = Assert.throws[IllegalArgumentException](matrix.matmul(MatrixD.zeros(2, 2)))
