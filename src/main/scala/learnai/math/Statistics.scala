...
final case class MeanEstimate(mean: Double, standardError: Double, lower: Double, upper: Double, samples: Int)
...
final case class ProbabilityForecast(probability: Double, outcome: Int):
  require(probability >= 0.0 && probability <= 1.0 && probability.isFinite, s"invalid probability: $probability")
...
final case class CalibrationBin(lower: Double, upper: Double, count: Int, meanPrediction: Double, eventRate: Double)
...
object Statistics:
  ...
  def sampleVariance(values: Vector[Double]): Either[String, Double] =
    if values.size < 2 then Left("sample variance requires at least two observations")
      ...
      val sumSquares = Numerics.compensatedSum(values.iterator.map(value => math.pow(value - mean, 2)))
      Right(sumSquares / (values.size - 1))

  /** Two-sided normal-approximation confidence interval for a sample mean. */
  def meanConfidenceInterval(values: Vector[Double], z: Double = 1.959963984540054): Either[String, MeanEstimate] =
    if z <= 0.0 || !z.isFinite then Left(s"z score must be finite and positive: $z")
      ...
      sampleVariance(values).flatMap { variance =>
        sampleMean(values).map { mean =>
          ...
          MeanEstimate(mean, standardError, mean - z * standardError, mean + z * standardError, values.size)
        }
      }
  ...
  def expectedCalibrationError(forecasts: Vector[ProbabilityForecast], binCount: Int): Either[String, Double] =
    if forecasts.isEmpty then Left("calibration error requires at least one forecast")
    else
      val bins = reliability(forecasts, binCount)
      Right(bins.map(bin => bin.count.toDouble / forecasts.size * math.abs(bin.meanPrediction - bin.eventRate)).sum)

  private def AssertFinite(result: Either[String, Double]): Double = result.fold(message => throw new IllegalArgumentException(message), identity)
...
def runStatisticsLab(): Unit =
  ...
  val forecasts = Vector(
    ProbabilityForecast(0.1, 0), ProbabilityForecast(0.2, 0),
    ProbabilityForecast(0.8, 1), ProbabilityForecast(0.9, 1)
  )
