 ...
 *
 * Chapter 24's cache reserves `maximumContextLength` rows per request up front, so a server's
 * request capacity is decided by the *worst-case* context even when most requests are short. Paged
 * storage (vLLM's PagedAttention insight) splits KV storage into fixed-size pages and hands them
 * out on demand: a request holds pages proportional to its *actual* length, and waste is bounded by
 * one partial page per sequence.
 *
 * The pool owns all storage and a reference count per page, so full pages can be shared between
 * sequences (prefix reuse). Two invariants make sharing safe without copy-on-write machinery:
 *
 *   - appends only ever write into a sequence's final, partially filled page;
 *   - [[PagedKvSequence.fork]] shares only *full* pages and copies the partial one, so any page
 *     with reference count above one is full and therefore immutable.
 *
 * Byte attribution lives here and not on sequences: with sharing, summing per-sequence footprints
 * double-counts, and the pool's allocated page count is the ground truth a scheduler must budget
 * against.
 */
final class KvPagePool(val pageCount: Int, val pageSize: Int, val channels: Int):
  ...
  def pagePayloadBytes: Long = 2L * pageSize.toLong * channels.toLong *
    java.lang.Double.BYTES.toLong

  /** Ground-truth allocated payload across all sequences, sharing included once. */
  def allocatedPayloadBytes: Long = Math.multiplyExact(allocatedPageCount.toLong, pagePayloadBytes)
  ...
  private[serving] def allocate(): Either[String, Int] =
    if freePages.isEmpty then Left(s"page pool exhausted: all $pageCount pages are allocated")
    else
  ...
  private[serving] def copyRow(fromPage: Int, toPage: Int, slot: Int): Unit =
 ...
 *
 * The page table maps logical position `p` to page `table(p / pageSize)`, slot `p % pageSize` — the
 * same indirection a paged attention kernel performs per score. Sequences are single-owner and
 * append-only; [[release]] returns pages to the pool and permanently retires the sequence, which is
 * the *mechanism* a serving scheduler's eviction or preemption policy (Chapter 27e) invokes.
 */
final class PagedKvSequence private (val pool: KvPagePool):
  ...
  /**
   * Internal fragmentation: reserved slots the sequence does not use. Bounded by `pageSize - 1` for
   * any sequence length — the whole point of small pages versus Chapter 24's full-context
   * reservation.
   */
  ...
  def append(key: Vector[Double], value: Vector[Double]): Either[String, Unit] =
    ...
    require(value.size == pool.channels, s"value width ${value.size} != channels ${pool.channels}")
    val slot      = currentLength % pool.pageSize
    val pageReady = if slot == 0 then pool.allocate().map(page => pageTable += page) else Right(())
    pageReady.map { _ =>
      pool.writeRow(pageTable.last, slot, key, value)
      currentLength += 1
    }

  def keyAt(position: Int, channel: Int): Double = pool
    .keyAt(pageFor(position), position % pool.pageSize, requireChannel(channel))

  def valueAt(position: Int, channel: Int): Double = pool
    .valueAt(pageFor(position), position % pool.pageSize, requireChannel(channel))

  /**
   * Creates a sequence sharing this one's full prefix pages.
   *
   * Full pages are shared by reference count; a partially filled final page is copied into a
   * freshly allocated page so both sequences can keep appending independently. On pool exhaustion
   * the fork fails atomically: every retained page is released and the parent is untouched.
   */
  ...
  private def requireActive(): Unit = require(active, "sequence has been released back to the pool")
