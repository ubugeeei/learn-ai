 ...
 *
 * Moments are stored by parameter *position*, so a snapshot is only valid for the same parameter
 * ordering it was captured from. Every value is validated on restore: first moments must be finite
 * and second moments finite and non-negative, because a corrupted checkpoint must fail loudly
...
object AdamWSnapshot:
  ...
  def zero(parameters: Vector[Tensor]): AdamWSnapshot = AdamWSnapshot(
    step = 0L,
...
object GradientNorm:
  def global(parameters: Vector[Tensor]): Double =
    ...
    val squared = parameters.iterator.flatMap(_.gradients.iterator)
      .map(gradient => gradient * gradient)
    Numerics.requireFinite(math.sqrt(Numerics.compensatedSum(squared)), "global gradient norm")

  def clippingScale(globalNorm: Double, maximumNorm: Option[Double]): Double = maximumNorm match
    case None          => 1.0
    case Some(maximum) =>
...
final class AdamW(
  ...
  final private case class State(firstMoment: Array[Double], secondMoment: Array[Double])
  ...
  def stepAtLearningRate(
    ...
    parameters.foreach { parameter =>
      ...
      parameter.updateParameter { (index, data, rawGradient) =>
        ...
        state.firstMoment(index) = beta1 * state.firstMoment(index) + (1.0 - beta1) * gradient
   ...
   *
   * A parameter that has never been stepped yields zero moments, matching the state
   * `stepAtLearningRate` would lazily create for it.
   */
  def snapshot(parameters: Vector[Tensor]): AdamWSnapshot =
    ...
    val moments = parameters.map { parameter =>
      Option(states.get(parameter)) match
        ...
        case None        => (Vector.fill(parameter.size)(0.0), Vector.fill(parameter.size)(0.0))
    }
    AdamWSnapshot(currentStep, moments.map(_._1), moments.map(_._2))

  /**
   * Replaces all optimizer state from a snapshot captured for the same parameter ordering.
   *
   * Every moment vector is validated for size, finiteness, and (for second moments) non-negativity
   * before any state is changed, so a rejected snapshot leaves the optimizer untouched.
   */
...
object Initialization:
  ...
  def heUniform(shape: Shape, fanIn: Int, random: SplittableRandom, label: String): Tensor =
    require(fanIn > 0, s"fan-in must be positive: $fanIn")
    val bound = math.sqrt(6.0 / fanIn.toDouble)
    uniform(shape, -bound, bound, random, label)

  def zeros(shape: Shape, label: String): Tensor = Tensor
    .parameter(shape, Array.fill(shape.size)(0.0), label)
  ...
  private def uniform(
    ...
    .parameter(shape, Array.fill(shape.size)(random.nextDouble(minimum, maximum)), label)
