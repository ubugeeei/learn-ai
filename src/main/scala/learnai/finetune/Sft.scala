 ...
 *
 * Contents are token IDs rather than strings on purpose: tokenization is Chapter 15's contract, and
 * keeping the template token-level makes every mask position exactly checkable in tests.
 */
 ...
 *
 * The shape rules are the ones most chat stacks enforce before training: an optional system message
 * first, no assistant opening, and no two consecutive messages from the same role. Rejecting
 * malformed data here — before rendering — is the "data validation" half of SFT; silently training
 * on a double-assistant turn corrupts the loss mask without any runtime error later.
 */
final case class Conversation(messages: Vector[ChatMessage]):
  ...
  messages.iterator.zip(messages.iterator.drop(1)).zipWithIndex.foreach {
    case ((previous, current), index) => require(
        previous.role != current.role,
        s"messages ${index} and ${index + 1} share the role ${current.role}"
      )
  }

  def assistantMessages: Vector[ChatMessage] = messages.filter(_.role == ChatRole.Assistant)
 ...
 *
 * `assistantSpan(i)` is true when token `i` is assistant *content* or the end-of-turn of an
 * assistant message. Role markers are never in the span: the model is conditioned on "the assistant
 * speaks next", not trained to decide it.
 */
final case class RenderedConversation(tokens: Vector[TokenId], assistantSpan: Vector[Boolean]):
...
object ChatTemplate:
   ...
   *
   * Contents must not contain any special token; a document that embeds a fake `endOfTurn` would
   * silently terminate the mask early, which is a prompt-injection shaped data bug this validation
   * refuses.
   */
  def render(conversation: Conversation, specials: ChatSpecialTokens): RenderedConversation =
  ...
  /**
   * Aligns a rendered conversation into a causal example that trains only on assistant spans.
   *
   * Position `t` predicts token `t + 1`, so the loss mask at `t` is the assistant-span flag of the
   * *target*. Every assistant message therefore contributes `content.size + 1` trainable
   * predictions (its content plus its end-of-turn), which the test suite asserts as a counting
   * invariant. Conversations without an assistant message cannot form a training example.
   */
  def trainingExample(rendered: RenderedConversation): SftExample = SftExample(
...
object SftEvaluation:
   ...
   *
   * Each conversation contributes its masked mean loss weighted by its trainable-target count, so
   * the result is the mean loss per assistant token — comparable across sets whose conversations
   * differ in length. Evaluation is deterministic: no sampling is involved anywhere.
   */
  def heldOutLoss(
      model: MiniGpt,
  ...
  def exampleLoss(model: MiniGpt, example: SftExample): Tensor = model
    .lossMasked(example.inputs, example.targets, example.lossMask)
