 ...
 *
 * Full fine-tuning updates every element of a weight matrix. LoRA instead freezes the base layer
 * and learns an additive update constrained to rank `r`:
 *
 * `h = base(x) + (alpha / r) * (x A) B`
 *
 * with `A: [in, r]` and `B: [r, out]`. The trainable element count drops from `in * out` to
 * `r * (in + out)`, and because the update is a plain matrix product, it can be *merged* into the
 * base weight after training, leaving inference cost identical to the original layer.
 *
 * Two initialization facts carry most of the method's safety story:
 *
 *   - `B` starts at zero, so a freshly wrapped layer computes exactly the base function —
 *     adaptation begins from "no change", not from noise;
 *   - `A` starts random, so gradients through `B` are non-degenerate from the first step.
 *
 * Freezing is an *optimizer-side* contract in this codebase: the base parameters still receive
 * gradients during backward (they are on the computation path), but [[trainableParameters]] exposes
 * only the adapter matrices, and only what the optimizer receives changes. The test suite pins the
 * base bitwise across training steps.
 */
final class LoraLinear private (
   ...
   *
   * The merged layer owns fresh parameter tensors and computes the same function as [[apply]] up to
   * floating-point association, with the adapter's runtime cost gone. The bias is copied unchanged
   * — LoRA never touches it.
   */
  def merged(label: String): Linear =
    ...
    Linear
      .fromValues(base.inputChannels, base.outputChannels, mergedWeights, base.bias.values, label)

object LoraLinear:
  /**
   * Wraps a base layer with a rank-`r` adapter in the published initialization: random down
   * projection, zero up projection.
   */
  def wrap(
      base: Linear,
      rank: Int,
