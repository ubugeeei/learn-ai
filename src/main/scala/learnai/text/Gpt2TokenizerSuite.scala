...
object Gpt2TokenizerSuite extends TestSuite:
  ...
  override val tests: Vector[TestCase] = specify(
    ...
    test("base byte artifacts round-trip multilingual strict UTF-8") {
      val tokenizer = Assert.right(
        Gpt2Tokenizer.fromArtifacts(encoderJson(baseArtifacts()), "#version: 0.2\n")
      )
    ...
    test("artifact parser rejects malformed IDs headers and duplicate merges") {
      ...
      Assert.isTrue(
        Gpt2Tokenizer.fromArtifacts("{\"a\":0}", "#version: 0.2\na b\na b\n").isLeft
      )
    },
  ...
  private def baseArtifacts(): Vector[String] =
    Vector.tabulate(256)(byte => Gpt2Tokenizer.byteSymbol(byte).toString)

  private def encoderJson(pieces: Vector[String]): String =
    JsonObject(pieces.zipWithIndex.map { case (piece, id) =>
      piece -> JsonNumber(BigDecimal(id))
    }).render
