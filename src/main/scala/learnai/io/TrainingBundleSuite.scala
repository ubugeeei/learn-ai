...
object TrainingBundleSuite extends TestSuite:
  ...
  private val trainingConfig = MiniGptTrainingConfig(
    ...
    learningRateSchedule =
      WarmupCosineLearningRate(peak = 0.03, minimum = 0.003, warmupUpdates = 3),
  ...
  override val tests: Vector[TestCase] = specify(
    test("a mid-run state survives the disk round trip exactly") {
      ...
      try
        ...
        val (_, state) = ResumableMiniGptTraining
          .train(model, split, trainingConfig, initial, updates = 3)
    ...
    test("resuming from disk matches the straight run bitwise") {
      ...
      try
        ...
        val (straightRun, _) = ResumableMiniGptTraining
          .trainFromStart(straightModel, split, trainingConfig)
        ...
        val (firstMetrics, middle) = ResumableMiniGptTraining
          .train(firstModel, split, trainingConfig, initial, updates = 4)
        ...
        val (secondMetrics, _) = ResumableMiniGptTraining
          .train(resumedModel, split, trainingConfig, restored, updates = 4)

        Assert.equal(firstMetrics ++ secondMetrics, straightRun.steps)
        Assert.equal(resumedModel.parameters.map(_.values), straightModel.parameters.map(_.values))
      finally
        val _ = Files.deleteIfExists(path)
        val _ = Files.deleteIfExists(directory)
    },
