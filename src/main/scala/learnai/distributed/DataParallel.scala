 ...
 *
 * Each rank owns a full model replica and a shard of every global batch; one gradient all-reduce
 * per update keeps the replicas identical. The simulation preserves the two properties that make
 * real data parallelism debuggable:
 *
 *   - *replica identity*: replicas start from the same seed, receive the same reduced gradients in
 *     the same order, and run identical optimizer steps, so their weights must stay bit-for-bit
 *     equal — any divergence is a bug, never noise;
 *   - *single-process equivalence*: microbatch loss is scaled by `microBatchSize / batchSize`
 *     exactly as in Chapter 22b, so the all-reduce **sum** of rank gradients is the mean-over-batch
 *     gradient, and results match the Chapter 22c trainer to rounding. The match is deliberately
 *     *not* bitwise: sequential accumulation interleaves element additions inside each backward
 *     pass, while ranks complete their partial gradients before the reduction adds them — a
 *     different association of the same terms. Chasing bitwise equality across parallelism
 *     strategies is a losing game; bitwise *replica identity within one strategy* is the invariant
 *     worth enforcing, and this simulation does.
 *
 * Gradients are flattened into one bucket per update (in parameter order), all-reduced once, and
 * written back with `assignGradients` — the same fuse-then-reduce structure real frameworks use to
  ...
  def train(
    ...
    while updateIndex < config.totalUpdates do
      ...
      val rankResults = Vector.tabulate(worldSize) { rank =>
        ...
        val owned         = microBatches.slice(rank * microBatchesPerRank, (rank + 1) * microBatchesPerRank)
        ...
        owned.foreach { examples =>
          ...
          loss.scale(microBatch.batchSize.toDouble / config.batchSize.toDouble)
