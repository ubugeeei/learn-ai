...
object DataParallelSuite extends TestSuite:
  ...
  private val split  = CausalDataset
    .contiguousSplit(tokens, trainingFraction = 0.75, contextLength = 3)

  private def trainingConfig(
      totalUpdates: Int,
      batchSize: Int,
      microBatchSize: Int
  ): MiniGptTrainingConfig = MiniGptTrainingConfig(
    ...
    learningRateSchedule =
      WarmupCosineLearningRate(peak = 0.03, minimum = 0.003, warmupUpdates = 2),
  ...
  override val tests: Vector[TestCase] = specify(
    test("data-parallel training matches the single-process trainer to rounding") {
      ...
      parallel.metrics.map(_.trainingLoss).zip(singleRun.steps.map(_.trainingLoss)).foreach {
        case (parallelLoss, singleLoss) => Assert.close(parallelLoss, singleLoss, tolerance = 1e-12)
      }
      parallel.metrics.map(_.gradientNorm).zip(singleRun.steps.map(_.gradientNorm)).foreach {
        case (parallelNorm, singleNorm) => Assert.close(parallelNorm, singleNorm, tolerance = 1e-12)
      }
      Assert.equal(parallel.metrics.map(_.learningRate), singleRun.steps.map(_.learningRate))
      singleModel.parameters.map(_.values).flatten
        .zip(parallel.replicas.head.parameters.map(_.values).flatten).foreach {
          case (single, replicated) => Assert.close(replicated, single, tolerance = 1e-10)
        }
    },
    test("replicas stay bitwise identical even with several microbatches per rank") {
      ...
      parallel.replicas.tail
        .foreach(replica => Assert.equal(replica.parameters.map(_.values), reference))
      ...
      singleModel.parameters.map(_.values).flatten
        .zip(parallel.replicas.head.parameters.map(_.values).flatten).foreach {
          case (single, replicated) => Assert.close(replicated, single, tolerance = 1e-9)
        }
    },
    test("collectives combine contributions in rank order with recorded traces") {
      val collectives = new Collectives(worldSize = 3)
      val sum         = collectives
        .allReduceSum(Vector(Vector(1.0, 2.0), Vector(10.0, 20.0), Vector(100.0, 200.0)))
      Assert.equal(sum, Vector(111.0, 222.0))
      val mean        = collectives.allReduceMean(Vector(Vector(3.0), Vector(6.0), Vector(9.0)))
      ...
      Assert.equal(
        collectives.traces.map(_.operation),
    ...
    test("training records one gradient bucket and one loss reduction per update") {
      ...
      gradientBuckets
        .foreach(trace => Assert.equal(trace.elementCount, parallel.replicas.head.parameterCount))
    ...
    test("invalid world sizes contributions and gradient assignments are rejected") {
      ...
      val wrongCount  = Assert
        .throws[IllegalArgumentException](collectives.allReduceSum(Vector(Vector(1.0))))
      ...
      val notTrainable = Assert
        .throws[IllegalArgumentException](constant.assignGradients(Vector(0.1, 0.2)))
      Assert.isTrue(notTrainable.getMessage.contains("trainable"))
      val parameter    = Tensor.parameter(Shape(2), Vector(1.0, 2.0), "x")
      val wrongSize    = Assert
        .throws[IllegalArgumentException](parameter.assignGradients(Vector(0.1)))
