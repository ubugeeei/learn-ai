 ...
 *
 * `logicalPayloadBytes` is the size of the reduced result every rank ends up holding.
 * `ringBytesPerRank` is what a chunked ring algorithm actually moves per rank to get there — the
 * number a network budget must use, because collectives cost bandwidth per *rank*, not per result.
 */
...
/**
 * Deterministic in-process simulation of the collectives one training step needs.
 *
 * Real collectives run over a network with nondeterministic arrival order; frameworks then fix a
 * reduction order so replicas stay bit-identical. This simulation keeps that essential property —
 * contributions are always combined in rank order 0, 1, ..., N-1 — while running in one process, so
 * tests can demand *bitwise* replica equivalence instead of tolerances. Every call appends a
...
final class Collectives(val worldSize: Int):
   ...
   *
   * Fixed order is the determinism contract: floating-point addition is not associative, so "sum in
   * arrival order" would make replicas diverge run to run even with identical inputs.
   */
   ...
   *
   * The ring splits the buffer into `worldSize` chunks of `ceil(elements / worldSize)` elements.
   * Reduce-scatter sends `worldSize - 1` chunks per rank and all-gather sends the same again:
   * `2 (N-1) * chunkBytes`, which approaches `2x` the buffer for large `N` — the reason gradient
   * buckets are sized in tens of megabytes rather than per tensor.
   */
  ...
  private def record(operation: String, elementCount: Int): Unit = log += CollectiveTrace(
    operation,
    worldSize,
    elementCount,
