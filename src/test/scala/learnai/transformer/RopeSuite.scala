...
object RopeSuite extends TestSuite:
  ...
  override val tests: Vector[TestCase] = Vector(
    ...
    test("position zero is the identity rotation") {
      ...
      rope.rotate(input, startPosition = 0).values.zip(values).foreach {
        case (actual, expected) => Assert.close(actual, expected, tolerance = 1e-12)
      }
    },
    ...
    test("rotated dot products depend only on the relative offset") {
      ...
      def rotatedDot(queryPosition: Int, keyPosition: Int): Double =
        val rotatedQuery =
          rope.rotate(Tensor.constant(Shape(1, 8), query), queryPosition).values
        val rotatedKey =
          rope.rotate(Tensor.constant(Shape(1, 8), key), keyPosition).values
    ...
    test("autodiff gradients through the rotation match finite differences") {
      ...
      def loss(values: Vector[Double]): Double =
        rope
          .rotate(Tensor.constant(Shape(2, 4), values), startPosition = 3)
    ...
    test("rotary attention keeps causal masking intact") {
      val attention =
        RotaryCausalSelfAttention.random(4, 2, new SplittableRandom(3L), "rotary")
    ...
    test("a common position offset does not change rotary attention output") {
      ...
      val attention =
        RotaryCausalSelfAttention.random(8, 2, new SplittableRandom(7L), "rotary")
    ...
    test("rotary attention has identical parameter count to standard attention") {
      ...
      Assert.equal(
        rotary.parameters.map(_.size).sum,
        standard.parameters.map(_.size).sum
      )
    },
    test("invalid construction and inputs are rejected") {
      val oddChannels = Assert.throws[IllegalArgumentException] {
        RotaryPositionEncoding.create(headChannels = 3)
      }
