...
/** One named, deferred executable specification. */
final case class TestCase(name: String, run: () => Unit)

/** Minimal declarative test DSL used by colocated feature specifications. */
trait TestSuite:
  def name: String
  def tests: Vector[TestCase]

  final protected def test(testName: String)(body: => Unit): TestCase =
    TestCase(testName, () => body)

  /** Declares the complete ordered behavior contract for a suite. */
  final protected def specify(cases: TestCase*): Vector[TestCase] = cases.toVector

/** Dependency-free assertions with diagnostics designed for numerical code. */
object Assert:
  def equal[A](actual: A, expected: A): Unit =
    if actual != expected then fail(s"expected <$expected> but got <$actual>")
  ...
  def throws[E <: Throwable: ClassTag](body: => Any): E =
    ...
    try
      ...
      case error: Throwable if expectedClass.isInstance(error) => error.asInstanceOf[E]
      case error: Throwable                                    => fail(s"expected ${expectedClass.getSimpleName} but got " + s"${error
